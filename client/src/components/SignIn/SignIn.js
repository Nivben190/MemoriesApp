...
const SignIn = (props) => {
  ...
  const {setUser,setMemories,profileImage,isProfile,setProfileImage} = useContext(UserContext);
...
const handleSubmit = async (e) => {
  ...
  try {  
    ...
    const {data: user } = await axios.post("http://localhost:8080/api/getusers",data);
     setUser(user.user);
     setProfileImage(user.user.profileImg);
     setDataToLocalStorage(res, user, profileImage);
     const{data:notes}=await axios.post("http://localhost:8080/api/getallnotes",data);
...
<div className={styles.app}>
    <h1>Welcome Back</h1>
   {isProfile&&<img  src={profileImage} alt="User Profile " className={styles.ProfileImg}/>}
    <form onSubmit={handleSubmit}>
      <div>
      <input required  className={styles.InputLogin}  value={data.email} name="email" onChange={handleChange} type="email" placeholder="Your Email"></input>
      </div>
      <div >
