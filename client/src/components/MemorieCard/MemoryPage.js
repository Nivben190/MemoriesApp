    ...
    useEffect(() => {
     ...
     useEffect(() => {
       const getMemories = async () => {   
         const array=JSON.parse(localStorage.getItem('userMemories'));
         setMemoriesStart(Array.from(array).map((memorie,index)=>{
           return <Grid item xs={12} md={6} lg={4}>
           <MemoryCard 
             ImgSrc={memorie.ImgSrc}
             Header={memorie.Header}
...
{     
  ...
  setItems(prevCards=>
    {
      return [...prevCards,<Grid item xs={12} md={6} lg={4}>
        <MemoryCard   
         ImgSrc={cardImage}
         Header={Card.Header}
         Body={Card.Body}
    ...
    <Grid container>
      <Grid item xs={12}>
      <UserContext.Provider value={{user,setUser,cardImage,setCardImage}}>
     <CardComponent
     addImage={addImage}
      onAdd={addCard}
      Header="Header"
